...
class ImageMapper():
    ...
    def __init__(self,
        ...
        :param mapping_method: algorithm to be used when converting hexagonal
                               pixel camera data to square images
        ...
        :param channels: if True, the number of input channels is 2 (charges and 
                         peak arrival times)
        """
        ...
        for camtype in self.camera_types:
            ...
            if map_method not in ['oversampling', 'rebinning', 'nearest_interpolation', 'bilinear_interpolation', 'bicubic_interpolation', 'image_shifting', 'axial_addressing']:
                raise ValueError("Hex conversion algorithm {} is not implemented.".format(map_met))
            elif map_method in ['image_shifting', 'axial_addressing'] and camtype in ['ASTRICam', 'CHEC', 'SCTCam']:
                raise ValueError("{} (hexagonal convolution) is not available for square pixel cameras.".format(map_met))
