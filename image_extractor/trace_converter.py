...
class TraceConverter:
    ...
    def convert_SCT(self,pixels_vector,peaks_vector):
        ...
        MODULE_START_POSITIONS = [(((image_shape[0]-MODULES_PER_ROW[j]*MODULE_DIM[0])/2)+(MODULE_DIM[0]*i),j*MODULE_DIM[1]) for j in range(ROWS) for i in range(MODULES_PER_ROW[j])]

        if self.dim_order == 'channels_first':
            im_array = np.zeros([self.num_channels,image_shape[0]*scale_factor,image_shape[1]*scale_factor], dtype=self.img_dtype)
        elif self.dim_order == 'channels_last':
            im_array = np.zeros([image_shape[0]*scale_factor,image_shape[1]*scale_factor,self.num_channels], dtype=self.img_dtype)
